...
# 修改服务文件中的用户和路径（可选）
if [ -n "$1" ]; then
    sed -i "s|User=Jaben|User=$1|g" "$SYSTEMD_DIR/$SERVICE_NAME.service"
    sed -i "s|WorkingDirectory=/vol1/1000/Smart-Network-Tool|WorkingDirectory=$1|g" "$SYSTEMD_DIR/$SERVICE_NAME.service"
    echo "已修改服务用户为: $1"
    echo "已修改工作目录为: $1"
fi
